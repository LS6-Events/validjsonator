...
func ValidationTagsToSchema(validationTag string) (tagSchema Schema, required bool) {
	validation := strings.Split(validationTag, ",")
	for _, v := range validation {
		...
		// Aliases
		case "iscolor":
			hexColor, _ := ValidationTagsToSchema("hexcolor")
			hsl, _ := ValidationTagsToSchema("hsl")
			hsla, _ := ValidationTagsToSchema("hsla")
			rgb, _ := ValidationTagsToSchema("rgb")
			rgba, _ := ValidationTagsToSchema("rgba")
		...
		case "country_code":
			iso3166_1_alpha2, _ := ValidationTagsToSchema("iso3166_1_alpha2")
			iso3166_1_alpha3, _ := ValidationTagsToSchema("iso3166_1_alpha3")
			iso3166_1_numeric, _ := ValidationTagsToSchema("iso3166_1_numeric")
		...
		if !fieldRequired {
			tagSchema.AllOf = append(tagSchema.AllOf, schema)
		} else {
			required = true
		}
	}

	return tagSchema, required
}
